...
if __name__ == "__main__":
    ...
    train_dataloader, test_dataloader = load_split_data(sales_dataset, STEP_LSTM_TRAIN_RATIO, STEP_BATCH_SIZE, s="Step-LSTM")
    n_episode = math.ceil(len(sales_dataset)/EPIS_LENGTH)+1

    
    # # Step-LSTM
    # step_lstm_model_path, step_lstm_model = step_lstm_train(device, train_dataloader, test_dataloader, STEP_EPOCH, STEP_LR, n_episode)

    # # Episode-LSTM
    # if we want to jump the train of LSTM
    ...
    dp_actor_model, dp_actor_model_path = TD3_train(device, env, rep_array, n_episode,dic)
    print(dp_actor_model_path)
    
    # TD3 evaluation


    TD3_evaluation(device, env, step_lstm_model_path, epis_lstm_model_path, dp_actor_model_path, TRAIN_EPIS_NUM, TEST_EPIS_NUM, dic)